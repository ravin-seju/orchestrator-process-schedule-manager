...
- **Actionable insights** — One-click filters in the header for Duplicates, Stale (no upcoming run), Same-minute Collisions, and Expiring (scheduled to auto-disable soon); clicking auto-narrows the folder picker to the affected folders.
- **Trigger lifecycle awareness** — Triggers whose stop date has already passed or falls within the next 14 days are flagged with a glowing amber hourglass in the inventory table and the day detail panel, and an amber dot on calendar bars and chips — exactly the set the Expiring metric counts; hover for the full end date. Any trigger with a stop date, however far out, also gets an "Ends <date> · Soft Stop|Kill" line in the day detail panel, so upcoming auto-disables aren't a surprise. Occurrence generation stops at the stop date, so an expiring trigger never shows runs it cannot perform.
- **Auto-disabled triggers surfaced** — Orchestrator disables a trigger once its stop date passes, which hides it behind the default Enabled filter. A dismissible banner reports how many, and jumps straight to them; in the inventory their Status reads `Auto-disabled` rather than a plain `Disabled` that looks like someone switched them off.
